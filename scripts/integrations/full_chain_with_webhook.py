...
class DbtPlatformClient:
    ...
    def trigger(self, job_id, cause=""):
        r = requests.post(f"{self.base}/jobs/{job_id}/run/",
                          headers=self.headers, json={"cause": cause})
        r.raise_for_status()
    ...
    def wait(self, run_id, interval=30):
        ...
        while True:
            ...
            if status == DbtRunStatus.SUCCESS: return True
...
dbt.wait(run)
pbi_refresh(os.environ["TENANT_ID"], os.environ["CLIENT_ID"],
            os.environ["CLIENT_SECRET"], "<workspace-id>", "<dataset-id>")
